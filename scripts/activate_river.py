...
# ---------- main ----------
def main() -> int:
    p = argparse.ArgumentParser(description="Activate a river (reads cross_id from pipeline YAML).")
    p.add_argument("--pipeline-config", required=True, help="Path to pipeline YAML (must contain cross_id or advanced.cross_id)")
    # account/env come from env vars; flags are optional override if you want to run locally with args
    ...
    url = f"{API_BASE}/v1/accounts/{account_id}/environments/{env_id}/rivers/{river_id}/activate_river"
    ...
    if resp.status_code not in (200, 201, 202):
        # Machine-readable failure for CI logs
